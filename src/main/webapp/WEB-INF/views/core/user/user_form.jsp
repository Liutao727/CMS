	            ...
	            <div class="col-sm-10 form-inline">
					    	...
					    	<span id="roles"<c:if test="${!empty bean.type && bean.type==0}"> style="display:none;"</c:if>>
					    		<c:forEach var="role" items="${roleList}">
					    			<label class="checkbox-inline"<c:if test="${role.rank < bean.rank}"> style="color:#bbb;"</c:if>><input type="checkbox" name="roleIds" value="${role.id}"<c:if test="${fnx:contains_cso(bean.roles,'id',role.id)}"> checked="checked"</c:if><c:if test="${role.rank < bean.rank}"> disabled data-disabled="true"</c:if>/>${role.name}(${role.rank})</label>
					    		</c:forEach>
					    	</span>
	            </div>
	          </div>
