...
<div class="content-header">
	<h1><s:message code="info.management"/> - <s:message code="${oprt=='edit' ? 'edit' : 'create'}"/><c:if test="${oprt=='edit'}"> <small>(<s:message code="info.status"/>: <s:message code="info.status.${bean.status}"/>, ID:${bean.id})</small></c:if></h1>
</div>
<div class="content">
		...
		<form class="form-horizontal" id="validForm" action="${oprt=='edit' ? 'update' : 'save'}.do" method="post">
			...
			<div class="box-header with-border">
				<div class="btn-toolbar">
			      ...
			      <button class="btn btn-default" type="button" onclick="window.open('${bean.url}');"<c:if test="${oprt=='create'||bean.status ne 'A'}"> disabled="disabled"</c:if>><s:message code="info.foreView"/></button>
					</div>
					<div class="btn-group">
						  ...
						  <c:when test="${field.name eq 'file'}">
						    ...
						    <div class="input-group">
						      ...
						      <span class="input-group-btn">
						      	...
						      	<span id="fileSwfButton"></span>
						      	<button class="btn btn-default" type="button" class="swfbutton"><s:message code="upload"/></button>
					      		<button class="btn btn-default" type="button" id="fileSwfCancel" onclick="fileSwfUpload.cancelQueue();" disabled><s:message code="cancel"/></button>
						     	</span>
						    </div>
