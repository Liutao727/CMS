		...
		<div class="box-body table-responsive">
			...
			<form class="form-inline" action="batch_update.do" method="post">
				...
				<div style="margin-bottom:10px;">
				  ...
				  <label class="checkbox-inline" for="zipUpload"><input type="checkbox" id="zipUpload"<c:if test="${isFtp}"> disabled="disabled"</c:if> onclick="swfupload.setFileTypes(this.checked?'*.zip':'*.*',this.checked?'ZIP Files':'All Files');swfupload.setUploadURL(this.checked?location.pathname.substring(0,location.pathname.lastIndexOf('/')+1)+'zip_upload.do;jsessionid=<%=request.getSession().getId()%>?_site=' + ($.cookie('_site')||''):'upload.do;jsessionid=<%=request.getSession().getId()%>?_site=' + ($.cookie('_site')||''));"/><s:message code="webFile.zipUpload"/></label>
				  </shiro:hasPermission>
				  <shiro:hasPermission name="core:web_file_${type}:upload">
				  <span id="fileSwfButton"></span><button class="btn btn-default swfbutton" type="button"><s:message code="upload"/></button>
				  <button class="btn btn-default" id="fileSwfCancel" type="button" onclick="swfupload.cancelQueue();" disabled="disabled"><s:message code="cancel"/></button>
