...
<div class="c-bar margin-top5">
  <span class="c-position"><s:message code="visitLog.sourceAnalysis"/></span>
...
<fieldset class="c-fieldset">
  ...
  <span id="radio">
    <input type="radio" id="radioToday" onclick="location.href='source_analysis.do?period=today';"<c:if test="${period eq 'today'}"> checked="checked"</c:if>/><label for="radioToday"><s:message code="visitLog.trafficAnalysis.today"/></label>
		<input type="radio" id="radioYesterday" onclick="location.href='source_analysis.do?period=yesterday';"<c:if test="${period eq 'yesterday'}"> checked="checked"</c:if>/><label for="radioYesterday"><s:message code="visitLog.trafficAnalysis.yesterday"/></label>
		<input type="radio" id="radioLast7Day" onclick="location.href='source_analysis.do?period=last7Day';"<c:if test="${period eq 'last7Day'}"> checked="checked"</c:if>/><label for="radioLast7Day"><s:message code="visitLog.trafficAnalysis.last7Day"/></label>
		<input type="radio" id="radioLast30Day" onclick="location.href='source_analysis.do?period=last30Day';"<c:if test="${period eq 'last30Day'}"> checked="checked"</c:if>/><label for="radioLast30Day"><s:message code="visitLog.trafficAnalysis.last30Day"/></label>
	</span>
	<script type="text/javascript">
	$("#radio").buttonset();
	</script>
	<form action="source_analysis.do" method="get" style="display:inline;">
	    ...
	    legend: {
	        ...
	        data: [
	            <c:forEach var="bean" varStatus="status" items="${sourceList}">
		            <c:choose>
									<c:when test="${'DIRECT' eq bean[0]}">'<s:message code="visitLog.source.DIRECT"/>'</c:when>
									<c:when test="${'OTHER' eq bean[0]}">'<s:message code="visitLog.OTHER"/>'</c:when>
	    ...
	    series : [
	        {
	            ...
	            data:[
									<c:forEach var="bean" varStatus="status" items="${sourceList}">
										{value:${bean[1]}, name:'<c:choose><c:when test="${'DIRECT' eq bean[0]}"><s:message code="visitLog.source.DIRECT"/></c:when><c:when test="${'OTHER' eq bean[0]}"><s:message code="visitLog.OTHER"/></c:when><c:otherwise><c:out value="${bean[0]}"/></c:otherwise></c:choose>'}<c:if test="${!status.last}">,</c:if>
									</c:forEach>
	            ],
	            itemStyle: {
	                emphasis: {
...
<table id="pagedTable" border="0" cellpadding="0" cellspacing="0" class="ls-tb margin-top5">
  ...
  <tr>
    ...
    <td>
    	<c:choose>
    		<c:when test="${'DIRECT' eq bean[0]}"><s:message code="visitLog.source.DIRECT"/></c:when>
    		<c:otherwise><a href="<c:out value="${bean[0]}"/>" target="_blank" style="color:blue;"><c:out value="${bean[0]}"/></a></c:otherwise>
    	</c:choose>
    </td>
